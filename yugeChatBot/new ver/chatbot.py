...
class YugeChatBot(object):
    ...
    def getDictionary(self):
        ...
        angryFile = "angry.txt"
        angryFileContent = codecs.open(fileRelativePath+angryFile, 'rb', 'utf-8').readlines()
    ...
    def playDictionaryWave(self):
        ...
        f = wave.open(r".\\sounds\\dict"+str(dictNum)+"\\"+self.currentChain+".wav","rb")  
    ...
    def startIntro(self, name):
        ...
        soundFile = Path(".\\sounds\\dict"+str(self.dictionaryNumber+1)+"\\"+self.currentChain+".wav") 
        if soundFile.is_file():
            self.playDictionaryWave()
        else:
            self.makeSpeech(self.dictionaries[self.dictionaryNumber][0].entry)

        reply = "ほお、"+self.userName+"、"+self.dictionaries[self.dictionaryNumber][0].entry
    ...
    def angryResponse(self):
        ...
        print("angry level = "+str(angryLevel)+", angry percentage = "+str(self.angryPercentage)+"%")
